...
### Констаны SCSS
Такие константы, как: цвета, медиа-миксины, функции пересчёта, константы шрифтов и тп должны находиться в папке ```front-scss/utils/``` и импортироваться в файл ```front-scss/utils/constants.scss```, который, в свою очередь импортируется во все файлы стилей. (В ином случае при сборке бандла вебпак выдаст ошибку, что какая-то константа в каком-то scss файле не найдена)
